...
RUN apt-get update \
  && apt-get upgrade -y \
  && apt-get install -y locales-all gettext libcurl4-gnutls-dev libpng-dev libssl-dev libc-client2007e-dev libkrb5-dev debconf-utils unzip cron re2c python tree libjpeg-dev libpng-dev wget \
  ...
  && docker-php-ext-install mysqli curl gd zip mbstring imap iconv gettext \
